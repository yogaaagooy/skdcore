...
export default function SimulationMenu() {
  ...
  return <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
    ...
    <main className="mx-auto max-w-5xl px-4 py-7 sm:py-10">
      ...
      <section className="mt-7 rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-sm font-bold">Paket soal 1–10</h2><p className="mt-1 text-xs text-slate-500">Paket 1 gratis. Paket 2–10 tersedia untuk pengguna premium.</p></div><span className={`rounded-full px-3 py-1 text-[11px] font-bold ${hasPremium ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}>{hasPremium ? "Premium aktif" : "Premium Rp29.000"}</span></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-5">{Array.from({length:10},(_,i)=>i+1).map(number=><button key={number} onClick={()=>choosePackage(number)} className={`rounded-xl border px-3 py-2.5 text-sm font-semibold transition hover:border-blue-500 hover:text-blue-600 dark:border-slate-700 ${number === 1 || hasPremium ? "border-emerald-300 bg-emerald-50 text-emerald-700 dark:bg-emerald-950/20" : "border-slate-200"}`}><span className="block">Paket {number}</span><small className="mt-0.5 block text-[10px] font-medium opacity-70">{number === 1 ? "Gratis" : hasPremium ? "✓ Terbuka" : "🔒 Premium"}</small></button>)}</div></section>
    </main>
