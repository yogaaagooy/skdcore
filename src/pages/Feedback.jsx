...
export default function Feedback() {
  ...
  return <div className="app-page min-h-screen bg-slate-50 dark:bg-slate-950 dark:text-slate-50">
    ...
    <main className="mx-auto max-w-4xl px-4 py-7 sm:py-10">
      ...
      <form onSubmit={submit} className="mt-6 border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 sm:p-7">
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="text-sm font-semibold">Jenis masukan
            <select value={form.type} onChange={(event) => setForm({ ...form, type: event.target.value })} className="mt-2 w-full border border-slate-300 bg-white px-3 py-3 font-normal dark:border-slate-700 dark:bg-slate-800">
          ...
          <label className="text-sm font-semibold">Judul
            <input maxLength={80} value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Ringkasan masukan" className="mt-2 w-full border border-slate-300 bg-white px-4 py-3 font-normal dark:border-slate-700 dark:bg-slate-800" />
          </label>
        </div>
        <label className="mt-4 block text-sm font-semibold">Isi masukan
          <textarea maxLength={1500} rows={5} value={form.message} onChange={(event) => setForm({ ...form, message: event.target.value })} placeholder="Jelaskan masukan secara jelas..." className="mt-2 w-full resize-y border border-slate-300 bg-white px-4 py-3 font-normal dark:border-slate-700 dark:bg-slate-800" />
        </label>
        ...
        {notice && <p className={`mt-4 p-3 text-sm ${notice.startsWith("✓") ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>{notice}</p>}
        <button disabled={loading} className="mt-5 bg-blue-600 px-5 py-3 text-sm font-bold text-white disabled:opacity-50">{loading ? "Memproses..." : "Kirim masukan"}</button>
      </form>

      <section className="mt-8">
        <h2 className="text-lg font-bold">Riwayat masukan</h2>
        <div className="mt-3 space-y-3">{loading && !items.length ? <p className="border border-slate-200 bg-white p-5 text-sm text-slate-500 dark:border-slate-800 dark:bg-slate-900">Memuat masukan...</p> : items.length ? items.map((item) => <article key={item.id} className="border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
        ...
        </article>) : <p className="border border-slate-200 bg-white p-5 text-sm text-slate-500 dark:border-slate-800 dark:bg-slate-900">Belum ada masukan yang dikirim.</p>}</div>
      </section>
    </main>
  </div>;
}
